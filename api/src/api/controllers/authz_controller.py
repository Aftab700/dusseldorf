...
@router.get("/_users_/all", response_model=List[str])
async def list_all_system_users(
    db: AsyncIOMotorClient = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
    """List all unique usernames in the system (Admin only)"""
    if "admin" not in current_user.get("roles", []):
        raise HTTPException(status_code=403, detail="Admin access required")

    # Get unique usernames from the users collection
    users = await db.users.find({}, {"username": 1, "_id": 0}).to_list(None)
...
async def get_permissions_for_zone(
    ...
    can_assign_roles: bool = await permission_service.has_at_least_permissions_on_zone(
        zone, current_user, Permission.ASSIGNROLES
    )

    if not can_assign_roles:
        logger.warning(
            f"User {current_user['preferred_username']} attempted to access permissions for zone {zone}"
        )
...
async def get_authz_permission(
    ...
    if user != current_user["preferred_username"] and can_assign_roles == False:
        logger.warning(
            f"User {current_user['preferred_username']} attempted to access permissions for user {user}"
        )
        raise HTTPException(status_code=403, detail="Unauthorized")
...
async def grant_permission(
    ...
    if not can_assign_roles:
        logger.warning(
            f"User {user_id} attempted to grant permissions to {perm_req.alias} on zone {zone} without proper permissions"
        )
        raise HTTPException(status_code=403, detail="Unauthorized")
    ...
    is_owner: bool = await permission_service.has_at_least_permissions_on_zone(
        zone, current_user, Permission.OWNER
    )

    if is_owner == False and permission_to_give == Permission.OWNER:
        logger.warning(
            f"Non-owner {user_id} attempted to grant owner permission to {perm_req.alias} on zone {zone}"
        )
        raise HTTPException(
            status_code=403, detail="Only owners can grant owner permissions"
        )

    # all good, set the permission
    query = {"fqdn": zone, "authz.alias": perm_req.alias}
    if await db.zones.find_one(query):
        # we found one, so we need to update the permission for this user
        update_action = {"$set": {"authz.$.authzlevel": permission_to_give}}
        ...
        push_action = {
            "$push": {
                "authz": {"alias": perm_req.alias, "authzlevel": permission_to_give}
            }
        }
    ...
    logger.info(
        f"User {user_id} granted permission {permission_to_give} to user {perm_req.alias} on zone {zone}"
    )
...
async def remove_permission(
    ...
    if not can_remove_users:
        logger.warning(
            f"User {user_id} attempted to remove user {user} permissions without ASSIGNROLES"
        )
        raise HTTPException(status_code=403, detail="Unauthorized")
    ...
    if not current_permission:
        logger.warning(
            f"User {user_id} attempted to remove user {user} permissions that do not exist"
        )
    ...
    current_user_is_owner: bool = (
        await permission_service.has_at_least_permissions_on_zone(
            zone, current_user, Permission.OWNER
        )
    )
    ...
    if permission_to_remove == Permission.OWNER and current_user_is_owner == False:
        logger.warning(
            f"Non-owner {user_id} attempted to remove user {user} owner permission"
        )
        raise HTTPException(
            status_code=403, detail="Only owners can remove owner permissions"
        )

    if not await db.zones.update_one(
        {"fqdn": zone}, {"$pull": {"authz": {"alias": user}}}
    ):
        logger.exception(
            f"Failed to revoke permissions for {user} on {zone} by {user_id}"
        )
