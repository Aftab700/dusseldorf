...
async def get_rules(
    ...
    all_zones = await db.zones.find(
        {"authz.alias": current_user["preferred_username"]}, {"_id": 0, "fqdn": 1}
    ).to_list(None)
    confirmed_zones = []
    for zone in all_zones:
        can_read: bool = await permission_service.has_at_least_permissions_on_zone(
            zone["fqdn"], current_user, Permission.READONLY
        )
    ...
    rules = (
        await db.rules.find({"zone": {"$in": confirmed_zones}})
        .skip(skip)
        .limit(limit)
        .to_list(None)
    )
...
async def get_rules_by_zone(
    ...
    if not can_read:
        logger.warning(
            f"User {current_user['preferred_username']} attempted to access zone {zone} rules"
        )
...
async def create_rule(
    ...
    can_rw: bool = await permission_service.has_at_least_permissions_on_zone(
        rule.zone, current_user, Permission.READWRITE
    )
    if not can_rw:
        logger.warning(
            f"User {current_user['preferred_username']} attempted to create rule for zone {rule.zone}"
        )
    ...
    priority_result = await db.rules.find(
        {"zone": rule.zone, "networkprotocol": rule.networkprotocol},
        {"_id": 0, "priority": 1},
    ).to_list(None)
    if priority_result:
        priority_only_list = []
        ...
        if rule.priority in priority_only_list:
            ...
            available_priorities = [
                ele
                for ele in range(1, MAX_PRIORITY + 1)
                if ele not in priority_only_list
            ]
            ...
            if len(available_priorities) > 0:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Unable to find an unused priority"
                )
...
async def update_rule_priority(
    ...
    can_read_write: bool = await permission_service.has_at_least_permissions_on_zone(
        zone, current_user, Permission.READWRITE
    )
    if can_read_write == False:
        logger.warning(
            f"User {current_user['preferred_username']} attempted to update zone {zone} rule {rule_id}"
        )
        raise HTTPException(status_code=403, detail="Forbidden")
    ...
    if update_result.modified_count != 1:
        logger.error(
            f"Failed to update rule {zone}/{rule_id} priority: modified_count: {update_result.modified_count}"
        )
        raise HTTPException(status_code=400, detail="Rule update failed")

    logger.info(
        f"User {current_user['preferred_username']} updated rule {zone}/{rule_id} priority to {priority.priority}"
    )
...
async def delete_rule(
    ...
    if not can_rw:
        logger.warning(
            f"User {current_user['preferred_username']} attempted to delete rule {zone}/{rule_id}"
        )
    ...
    if result.deleted_count != 1:
        logger.exception(
            f"Failed to delete rule {zone}/{rule_id}: deleted_count: {result.deleted_count}"
        )
        raise HTTPException(status_code=400, detail="Failed to delete rule")

    logger.info(
        f"User {current_user['preferred_username']} deleted rule {zone}/{rule_id}"
    )
    return {"status": "success"}
...
async def add_rule_component(
    ...
    if not await permission_service.has_at_least_permissions_on_zone(
        ...
        logger.warning(
            f"User {current_user['preferred_username']} attempted to add components for zone {zone} rule {rule_id}"
        )
        raise HTTPException(status_code=403, detail="Unauthorized")
    ...
    update_result = await db.rules.update_one(
        {"zone": zone, "ruleid": UUID(rule_id)},
        {"$push": {"rulecomponents": new_component}},
    )
    if update_result.modified_count != 1:
        raise HTTPException(status_code=400, detail="Failed to add rule component")

    logger.info(
        f"User {current_user['preferred_username']} added zone {zone} rule {rule_id} component {new_component['componentid']}"
    )
...
async def edit_rule_component(
    ...
    if not can_write:
        logger.warning(
            f"User {current_user['preferred_username']} attempted to modify zone {zone} rule {rule_id} component {component_id}"
        )
        raise HTTPException(status_code=403, detail="Forbidden")
    ...
    update_result = await db.rules.update_one(
        {
            "zone": zone,
            "ruleid": UUID(rule_id),
            "rulecomponents.componentid": UUID(component_id),
        },
...
async def delete_rule_component(
    ...
    if not await permission_service.has_at_least_permissions_on_zone(
        ...
        logger.warning(
            f"User {current_user['preferred_username']} attempted to delete zone {zone} rule {rule_id} component {component_id}"
        )
        raise HTTPException(status_code=403, detail="Forbidden")
    ...
    logger.info(
        f"User {current_user['preferred_username']} deleted zone {zone} rule {rule_id} component {component_id}"
    )
    return {"status": "success"}
