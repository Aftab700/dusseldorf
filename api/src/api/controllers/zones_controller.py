...
async def get_zones(
    ...
    # Admin bypass: If not admin, filter by user alias
    if "admin" not in current_user.get("roles", []):
        query["authz.alias"] = current_user["preferred_username"]
    user_accessible_zones = (
        await db.zones.find(query).skip(skip).limit(limit).to_list(None)
    )

    if "admin" in current_user.get("roles", []):
        return [Zone(**zone) for zone in user_accessible_zones]
    ...
    for zone_check in user_accessible_zones:
        if await permission_service.has_at_least_permissions_on_zone(
            zone_check["fqdn"], current_user, Permission.READONLY
        ):
...
async def get_zone(
    ...
    can_read: bool = await permission_service.has_at_least_permissions_on_zone(
        fqdn, current_user, Permission.READONLY
    )

    if not can_read:
        logger.warning(
            f"User {current_user['preferred_username']} attempted to access zone {fqdn}"
        )
...
async def create_zone(
    ...
    # Global Role Check: Only users with 'admin' or 'owner' roles can create zones
    if "admin" not in user_roles and "owner" not in user_roles:
        logger.warning(
            f"403: User {user_id} without required roles attempted to create a zone"
        )
        raise HTTPException(
            status_code=403, detail="Insufficient global permissions to create zones"
        )
    ...
    if req.domain:
        if not await permission_service.validate_user_domain(user_id, req.domain):
            logger.warning(
                f"403: User {user_id} attempted to create zone({req.zone}) on domain({req.domain})"
            )
            raise HTTPException(status_code=403, detail="Unauthorized")
        domain = req.domain
    ...
    if req.zone:
        ...
        for existing_zone in all_zones:
            ...
            if new_zone_fqdn.endswith("." + lower_zone) or new_zone_fqdn == lower_zone:
                logger.warning(
                    f"403: User {user_id} attempted to create zone({req.zone}), which is a subdomain of {existing_zone['fqdn']}"
                )
        ...
        zone_request_dict = {
            ...
            "domain": domain,
            "expiry": None,
            "authz": [{"alias": user_id, "authzlevel": Permission.OWNER}],
        }
    ...
    if req.num > 0 and req.num <= MAX_ZONES:
        ...
        for i in range(req.num):
            ...
            while True:
                ...
                validation_filter = {"fqdn": new_zone_name}
            ...
            if result.inserted_id:
                logger.warning(
                    f"User {current_user['preferred_username']} created domain {domain} zone {new_zone_name}"
                )
...
async def delete_zone(
    ...
    is_owner = await permission_service.has_at_least_permissions_on_zone(
        fqdn, current_user, Permission.OWNER
    )
    if not is_owner:
        logger.warning(
            f"User {current_user['preferred_username']} attempted to delete zone {fqdn}"
        )
        raise HTTPException(status_code=403, detail="Unauthorized")
    ...
    if await db.rules.find_one({"zone": fqdn}):
        raise HTTPException(
            status_code=400, detail="Cannot delete zone with existing rules"
        )
