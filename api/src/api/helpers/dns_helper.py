...
class DnsHelper:
    ...
    def validate_zone_name(zone_name: str, domain: str) -> bool:
        ...
        try:
            if (
                zone_name.startswith(".")
                or zone_name.endswith(".")
                or ".." in zone_name
            ):
                return False
    ...
    def validate_mx_record(content: str) -> bool:
        ...
        try:
            ...
            return (
                priority.isdigit()
                and 0 <= int(priority) <= 65535
                and validate_domain_name(hostname)
            )
        except:
            return False
    ...
    def validate_srv_record(content: str) -> bool:
        ...
        try:
            ...
            return (
                all(x.isdigit() for x in [priority, weight, port])
                and 0 <= int(priority) <= 65535
                and 0 <= int(weight) <= 65535
                and 0 <= int(port) <= 65535
                and validate_domain_name(target)
            )
        except:
            return False
    ...
    def validate_caa_record(content: str) -> bool:
        ...
        try:
            ...
            return (
                flag.isdigit()
                and 0 <= int(flag) <= 255
                and tag in ["issue", "issuewild", "iodef"]
                and len(value) <= 255
            )
        except:
            return False
