...
class PermissionService:
    ...
    async def has_at_least_permissions_on_zone(
        self, zone: str, user_info: dict, min_permission: IntEnum = Permission.READONLY
    ) -> bool:
        """Check if user has at least the specified permission level on zone"""
        logger.error(
            f"has_at_least_permissions_on_zone({zone}, {user_info['preferred_username']}, {min_permission})"
        )
        if isinstance(user_info, str):
            user_info = {"preferred_username": user_info, "roles": []}
        ...
        perm_check = await self.db.zones.find_one(
            {"fqdn": zone, "authz.alias": user_id},
            {
                ...
                "authz": {
                    "$elemMatch": {
                        "alias": user_id,
                        "authzlevel": {"$gte": min_permission},
                    }
                },
            },
        )
        # return len(perm_check)
        return bool(perm_check)  # Return True if document found
    ...
    async def validate_user_domain(self, user: str, domain: str) -> bool:
        query = {
            "$or": [
                {"domain": domain, "users": {"$in": [user]}},
                {"domain": domain, "owner": user},
                {"domain": domain, "owner": "dusseldorf"},
            ]
        }
