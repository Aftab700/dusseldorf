...
class RequestProcessor:
    ...
    async def process_request(self, request_id: str):
        ...
        try:
            ...
            await self.db.dns_requests.update_one(
                {"_id": request_id}, {"$set": {"status": RequestStatus.PROCESSING}}
            )
            ...
            await self.db.dns_requests.update_one(
                {"_id": request_id},
                {"$set": {"status": RequestStatus.FAILED, "error_message": str(e)}},
            )
    ...
    async def _update_rule(self, zone_id: str, rule_data: dict):
        ...
        await self.db.rules.update_one(
            {"_id": rule_id, "zone_id": zone_id}, {"$set": rule_data}
        )

    async def _delete_rule(self, zone_id: str, rule_id: str):
        """Delete a DNS rule"""
        await self.db.rules.delete_one({"_id": rule_id, "zone_id": zone_id})
