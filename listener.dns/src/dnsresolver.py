...
class DusseldorfResolver(BaseResolver):
    ...
    def resolve(self, dnsrecord, handler) -> dnslib.DNSRecord:
        ...
        """
        # TODO: #BYOD
        reserved = [domain, f"ns1.{domain}", f"ns2.{domain}"]
        ...
        if qname_s in reserved:
            ...
            if resp := DnsResponse.forZone(req):
                ...
                logger.warning(
                    f"could not make RR for zone request {qtype_s}/{request_fqdn}"
                )
                reply.header.set_rcode(NXDOMAIN)
            return reply
        # endregion
        ...
        logger.debug(
            "DNSlistener resp: %.3f s, DB write: %.3f s", request_time, db_write_time
        )
        return reply
    ...
    def make_resource_record(self, response: DnsResponse) -> dnslib.RR:
        ...
        def _RR(rdata: any, ttl: int = 60) -> RR:
            ...
            return RR(rname=rname, rtype=rtype, rclass=1, ttl=ttl, rdata=rdata)
