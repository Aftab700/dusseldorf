...
class SetVariableResult(Result):
    ...
    def execute(cls, result_data: dict, parameter: str):
        ...
        if isinstance(result_data["response"], HttpResponse):
            ...
            result_data["response"].body = result_data["response"].body.replace(
                rename_from, replace_with
            )
            for key in result_data["response"].headers:
                if rename_from in result_data["response"].headers[key]:
                    result_data["response"].headers[key] = (
                        result_data["response"]
                        .headers[key]
                        .replace(rename_from, replace_with)
                    )
...
class GetHTTPPassthruResult(Result):
    ...
    def execute(cls, result_data: dict, parameter: str):
        ...
        try:
            ...
            logger.info(
                f"http.passthru for {target.netloc}{orig_req.path} gave {resp.status_code} ({len(resp.text)} bytes)"
            )
...
class GetHTTPPassthru2Result(Result):
    ...
    def execute(cls, result_data: dict, parameter: dict):
        ...
        tls_verify: bool = not parameter.get(
        ...
        for change_from, change_to in subs.items():
            ...
            for hdr in headers:
                if change_from in headers[hdr]:
                    ...
                    logger.debug(
                        f"http.passthru2: replaced {change_from} in http header: {hdr}"
                    )
        ...
        try:
            ...
            logger.info(
                f"http.passthru2 for {target.netloc}{orig_req.path} gave {resp.status_code} ({len(resp.text)} bytes)"
            )
...
class WeightedRandomSelectionResultWrapper(Result):
    ...
    def execute(cls, result_data: dict, parameter: str):
        ...
        sampled_result = random.choices(
            parameter["results"], weights=parameter["weights"], k=1
        )[0]
