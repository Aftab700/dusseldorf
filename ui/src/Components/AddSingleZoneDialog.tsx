...
const generateRandomSubdomain = (): string => {
    ...
    do {
        ...
        /^\d/.test(randomSubdomain) ||
        badWordsFilter.isProfane(randomSubdomain)
    ); // If randomSubdomain starts with a number or is profane, run again
...
export const AddSingleZoneDialog = ({
    ...
    return (
        <Dialog
            ...
            onOpenChange={(_, data) => {
                if (
                    data.type == "backdropClick" ||
                    data.type == "escapeKeyDown"
                ) {
                    onDismiss();
                }
            }}
            ...
            <DialogSurface style={{ width: 480 }}>
                <DialogBody>
                    ...
                    <DialogContent className="stack vstack-gap">
                        <Text>
                            Choose a desired zone name; it cannot already be
                            taken.
                        </Text>
                        <div className="stack hstack-center">
                            <Field
                                validationMessage={
                                    !isChecking && subdomainErrorMsg
                                        ? "This zone is already taken."
                                        : ""
                                }
                                validationState={
                                    !isChecking && subdomainErrorMsg
                    ...
                    <DialogActions>
                        <Button
                            ...
                            icon={<AddRegular />}
                            disabled={
                                errorMsg || isChecking || subdomainErrorMsg
                            }
                            onClick={() => {
                        ...
                        <Button icon={<DismissRegular />} onClick={onDismiss}>
                            Close
                        </Button>
                    </DialogActions>
                </DialogBody>
