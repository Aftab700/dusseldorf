...
export const RequestTable = ({
    ...
    const [visibleColumns, setVisibleColumns] = useState<
        TableColumnDefinition<DssldrfRequest>[]
    >([]);
    ...
    useEffect(() => {
        const visible = allColumns.filter((col) =>
            columnConfig.find(
                (config) => config.id === col.columnId && config.visible,
            ),
        );
    ...
    const [selectedRows, setSelectedRows] = useState(
        new Set<TableRowId>(request ? [JSON.stringify(request)] : []),
    );
    const onSelectionChange: DataGridProps["onSelectionChange"] = (_, data) => {
        if (data.selectedItems.size > 0) {
            ...
            if (next_req) {
                try {
                    setRequest(
                        JSON.parse(next_req as string) as DssldrfRequest,
                    );
                } catch {
    ...
    if (requests.length === 0) {
        return (
            <Text wrap style={{ wordWrap: "break-word" }}>
                No requests were found for <b>{zone}</b> When new network
                traffic is detected for <b>{zone}</b>, it will appear here.
            </Text>
        );
    }
        ...
        <div>
            ...
            >
                ...
                <DataGridBody<Request>>
                    {({ item, rowId }) => (
                        <DataGridRow<Request> key={rowId} selectionCell={null}>
                            {({ renderCell }) => (
                                <DataGridCell>
                                    <TableCellLayout truncate>
                                        {renderCell(item)}
            ...
            <div className="stack hstack-gap" style={{ paddingTop: 20 }}>
                <Dropdown
                ...
                <Tooltip content={`Next ${num} requests`} relationship="label">
                    <Button
                        appearance="subtle"
