...
const createDictionary = (
    isPredicate: boolean,
): Record<string, { label: string; onlyOne: boolean }> => {
    return {
        "dns.data": {
            label: "DNS Data",
            onlyOne: true,
...
export const RuleComponentMenu = ({
    ...
    const [dictionary] = useState<
        Record<string, { label: string; onlyOne: boolean }>
    >(createDictionary(isPredicate));
    const [itemList, setItemList] = useState<string[]>(
        getItemList(isHttp, isPredicate),
    );
        ...
        <Menu>
            <MenuTrigger disableButtonEnhancement>
                <MenuButton
                    ...
                    icon={<AddRegular />}
                    disabled={
                        isDisabled || // dns results dropdown, disabled if we do not yet have a type
                        (isHttp == false &&
                            isPredicate == false &&
                            ruleComponents.find(
                                (value) => value.actionname === "dns.type",
                            ) == undefined)
                    }
                >
