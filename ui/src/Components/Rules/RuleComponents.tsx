...
const actionValueToNiceString = (actionname: string, actionvalue: string) => {
    switch (actionname) {
        ...
        case "http.code": {
            ...
            const exp = Constants.HttpResultCodes.find(
...
export const RuleComponents = ({
    ...
    const [ruleComponents, setRuleComponents] = useState<RuleComponent[]>(
        rule.rulecomponents,
    );
    const [editComponentId, setEditComponentId] = useState<string | null>(null);
    const [predicateToCreate, setPredicateToCreate] = useState<string | null>(
        null,
    );
        ...
        createTableColumn<RuleComponent>({
            ...
            renderCell: (component) => {
                ...
                if (component.ispredicate) {
                    name =
                        predicateDictionary[component.actionname] ??
                        component.actionname;
                } else {
                    name =
                        resultDictionary[component.actionname] ??
                        component.actionname;
                }
                return name;
            },
        ...
        createTableColumn<RuleComponent>({
            ...
            renderCell: (component) => {
                if (component.componentid === editComponentId) {
                    if (
                        !component.ispredicate &&
                        component.actionname == "http.header"
                    ) {
                        ...
                        return (
                            <ResultHeaderRuleComponentEdit
                                ...
                                onSave={(newValue) => {
                                    DusseldorfAPI.EditRuleComponent(
                                        rule,
                                        component,
                                        newValue,
                                    )
                    ...
                    <Tooltip content="Delete" relationship="label">
                        <Button
                            ...
                            onClick={() => {
                                ...
                                DusseldorfAPI.DeleteRuleComponent(
                                    rule,
                                    component,
                                )
                                    .then(() => {
                                    ...
                                    .finally(() => {
                                        ...
                                        DusseldorfAPI.GetRuleDetails(
                                            rule.zone,
                                            rule.ruleid,
                                        )
                                            .then((updatedRule) => {
                                                setRuleComponents(
                                                    updatedRule.rulecomponents,
                                                );
                                            })
                                            .catch((err) => {
                                                Logger.Error(err);
    ...
    return (
        <div className="stack vstack-gaplarge">
            <Text>
                A rule has one or more components which are either a{" "}
                <strong>Filter</strong> or an <strong>Action</strong>. Use the
                controls below to add new components:
            </Text>
            ...
            <div className="stack vstack-gap" style={{ paddingLeft: 10 }}>
                {predicateToCreate && (
                    <GenericRuleComponent
                        isPredicate={true}
                        ...
                        onSave={(newValue) => {
                            if (predicateToCreate == "dns.type") {
                                ...
                                DusseldorfAPI.AddRuleComponent(rule, {
                                    ...
                                    .then((newComponent) => {
                                        setRuleComponents(
                                            ruleComponents.concat(newComponent),
                                        );
                                        updateSelectedRule(rule);
                                    })
                                    .catch((err) => {
                ...
                {ruleComponents.filter((c) => c.ispredicate).length === 0 && (
                    <>
                        <MessageBar intent="warning">
                            <Text wrap>
                                You have no filter(s) set, this rule will not
                                trigger. Use the <b>Add Filter</b> menu above to
                                add one.
                            </Text>
                        </MessageBar>
                        <Divider
            ...
            <div className="stack vstack-gap" style={{ paddingLeft: "10px" }}>
                ...
                {resultToCreate &&
                    resultToCreate != "http.header" &&
                    resultToCreate != "dns.data" && (
                        <GenericRuleComponent
                            isPredicate={false}
                            actionName={resultToCreate}
                            onDismiss={() => {
                ...
                {ruleComponents.filter((c) => !c.ispredicate).length === 0 && (
                    <MessageBar intent="warning">
                        <Text wrap>
                            You have no actions set, this rule will do nothing
                            and just give the default response. Use the{" "}
                            <b>Add Result</b> menu above to add one.
                        </Text>
                    </MessageBar>
                )}
            </div>
