...
export const DnsDataComponent = ({
    onSave,
    onDismiss,
    oldValue,
    dnsType,
}: DnsDataComponentProps): JSX.Element => {
    ...
    const parseKey = (v: string) => {
        try {
            const parsed = JSON.parse(v) as DnsData;
            return (
                Object.keys(parsed).find((k) =>
                    ["ip", "cname", "txt"].includes(k),
                ) ?? "ip"
            );
        } catch {
            return "ip";
    ...
    const [value, setValue] = useState<string>(
        oldValue ? parseValue(oldValue) : "duSSeldoRF",
    );
    const [jsonKey] = useState<string>(
        oldValue ? parseKey(oldValue) : getKey(dnsType),
    );

    return (
        <div className="stack hstack-center">
            <Field label={oldValue !== undefined ? "" : "DNS Response Data"}>
                <Input
                    value={value}
                    onChange={(_, data) => {
                        setValue(data.value);
