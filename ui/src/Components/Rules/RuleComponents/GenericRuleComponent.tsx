...
export const GenericRuleComponent = ({
    ...
    const [value, setValue] = useState<string>(
        getDefaultValue(isPredicate, actionName, oldValue),
    );
    const [invalidMessage, setInvalidMessage] = useState<string>(
        getInvalidMessage(isPredicate, actionName),
    );
    const [fieldLabel, setFieldLabel] = useState<string>(
        getFieldLabel(isPredicate, actionName, oldValue),
    );
    const [component, setComponent] = useState<JSX.Element>(<></>);

    const getElement = (isPredicate: boolean, actionName: string) => {
        if (!isPredicate && actionName === "http.body") {
            return <TextareaRuleComponent value={value} setValue={setValue} />;
