...
export const RuleTable = ({
    ...
    const [selectedRows, setSelectedRows] = useState(
        new Set<TableRowId>(ruleId ? [ruleId] : []),
    );
        ...
        return (
            <div className="stack vstack-gap">
                <MessageBar intent="info">
                    <Text wrap>
                        <strong>Info:</strong> No rules are setup for this zone
                        yet.
                    </Text>
                </MessageBar>

                <Text wrap style={{ wordWrap: "break-word" }}>
                    Create a new rule to capture network requests sent to{" "}
                    <strong>{zone}</strong>. Use rules to provide automated
                    responses to particular network request, based on your
                    filters.
                </Text>

                <Text wrap>
                    Each rule has a certain numeric <strong>priority</strong>{" "}
                    which defines the order all rules are executed for this
                    zone/protocol combination.
                </Text>
            </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <DataGridBody<Rule>>
                {({ item, rowId }) => (
                    <DataGridRow<Rule> key={rowId} selectionCell={null}>
                        {({ renderCell }) => (
                            <DataGridCell>
                                <TableCellLayout truncate>
                                    {renderCell(item)}
