...
export const CreateAsRuleDialog = ({
    rules,
}: CreateAsRuleProps): JSX.Element => {
    ...
    const [zones] = useState<Zone[]>(
        CacheHelper.GetZones().sort((a, b) => a.fqdn.localeCompare(b.fqdn)),
    );
    ...
    const addNewRules = (): void => {
        for (const newRule of newRules) {
            DusseldorfAPI.AddRule(
                fqdn,
                newRule.networkprotocol,
                newRule.priority,
                newRule.name,
            )
                .then((rule) => {
                    newRule.rulecomponents?.forEach((newComponent) => {
                        DusseldorfAPI.AddRuleComponent(
                            rule,
                            newComponent,
                        ).catch((err) => {
                            throw err;
            ...
            <DialogSurface className={styles.surface}>
                <DialogBody>
                    ...
                    <DialogContent className="stack vstack-gap">
                        <Text>
                            Create a rule based on this template as a rule for
                            the zone below:
                        </Text>
                        ...
                        {showWarning && (
                            <MessageBar>
                                <MessageBarBody>
                                    This will merge with existing rules on{" "}
                                    <b>{fqdn}</b>.
                                </MessageBarBody>
                            </MessageBar>
                        )}
                    ...
                    <DialogActions>
                        <Button
                            appearance="primary"
                            disabled={
                                !fqdn || newRules.length === 0 || showAdded
                            }
                            onClick={() => {
                        ...
                        <DialogTrigger disableButtonEnhancement>
                            <Button appearance="secondary" disabled={showAdded}>
                                Cancel
                            </Button>
                        </DialogTrigger>
                    </DialogActions>
