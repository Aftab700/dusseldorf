...
export const TemplateDetails = ({
    template,
}: ITemplateDetailsProps): JSX.Element => {
    ...
    const parseTemplateRules = (newPayload: string): void => {
        try {
            ...
            if (ex instanceof YAMLException) {
                setPayloadErrorMsg(
                    `Parsing Error on (${ex.mark.line}:${ex.mark.column}): ${ex.reason}`,
                );
    ...
    if (activeTemplate) {
        return (
            <div className="stack vstack-gap">
                ...
                {payloadErrorMsg && (
                    <MessageBar intent="error">{payloadErrorMsg}</MessageBar>
                )}
        ...
        return (
            <div className="stack vstack-gap">
                <Text>
                    Templates make up one or more rules that can be applied to a
                    zone.
                </Text>

                <Text>
                    For more information on how to use templates, please see the
                    documentation on{" "}
