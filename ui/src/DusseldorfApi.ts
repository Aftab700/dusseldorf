...
export class DusseldorfAPI {
    static makeRequest = (
        ...
        const full_url: string = `${DusseldorfConfig.api_host}/${url}`.replace(
            /([^:]\/)\/+/g,
            "$1",
        );
    ...
    /**
     * POST /auth/login: Exchange credentials for an opaque session token.
     */
    static Login = async (
        username: string,
        password: string,
        ...
        const full_url = `${DusseldorfConfig.api_host}/auth/login`.replace(
            /([^:]\/)\/+/g,
            "$1",
        );
        ...
        CacheHelper.SetToken(data.access_token);

        // Fetch user profile via heartbeat/ping
        const pingResp = await this.get("ping");
        if (pingResp.ok) {
            const pingData = await pingResp.json();
            CacheHelper.SetUser(pingData.user);
        }
    };
    // ------------------ API calls ------------------
    ...
    static GetRequests = async (
        ...
        if (!zone) {
            throw Error(
                `API.GetRequests(${zone}, ${num}, ${skip}, ${protocols}) bad arguments`,
            );
        }
        ...
        if (num > MAX_REQS) {
            Logger.Info(
                `Reducing # of GetRequests(${zone}) from ${num} to ${MAX_REQS}`,
            );
            num = MAX_REQS;
        }

        return this.get(
            `requests/${zone}?limit=${num}&skip=${skip}&protocols=${protocols}`,
        )
            .then((resp) => {
                if (resp.ok) {
                    return resp.json();
                    ...
                    throw Error(
                        `API.GetRequests(${zone}, ${num}, ${skip}, ${protocols}) failed`,
                    );
                }
            })
            .then((data) => {
        ...
        return this.post("rules", payload)
            .then((resp) => {
                if (resp.ok) {
                    ...
                    throw Error(
                        `API.AddRule(${zone}, ${protocol}, ${priority}, ${name}) failed`,
                    );
                }
            })
            .then((data) => {
    ...
    static UpdateRule = async (rule: Rule, priority: number): Promise<Rule> => {
        ...
        if (!rule.zone || !rule.ruleid) {
            throw Error(
                `API.UpdateRule(${rule.zone}, ${rule.ruleid}) bad arguments`,
            );
        }

        return this.put(`rules/${rule.zone}/${rule.ruleid}`, priority).then(
            (resp) => {
                if (resp.ok) {
                    ...
                    throw Error(
                        `API.UpdateRule(${rule.zone}, ${rule.ruleid}) failed`,
                    );
                }
            },
        );
    ...
    static DeleteRule = async (rule: Rule): Promise<void> => {
        ...
        if (!rule.zone || !rule.ruleid) {
            throw Error(
                `API.DeleteRule(${rule.zone}, ${rule.ruleid}) bad arguments`,
            );
        }
        ...
        return this.delete(`rules/${rule.zone}/${rule.ruleid}`).then((resp) => {
            if (!resp.ok) {
                throw Error(
                    `API.DeleteRule(${rule.zone}, ${rule.ruleid}) failed`,
                );
            }
        });
    };
     ...
     */
    static AddRuleComponent = (
        rule: Rule,
        component: NewRuleComponent,
    ): Promise<RuleComponent> => {
        Logger.Info(
            `API.AddRuleComponent(${rule.zone}, ${rule.name}, ${component.actionname})`,
        );

        if (!rule.zone || !rule.ruleid) {
            throw Error(
                `API.AddRuleComponent(${rule.zone}, ${rule.name}, ${component.actionname}) bad arguments`,
            );
        }

        return this.post(
            `rules/${rule.zone}/${rule.ruleid}/components`,
            component,
        )
            .then((resp) => {
                if (resp.ok) {
                    ...
                    throw Error(
                        `API.AddRuleComponent(${rule.zone}, ${rule.name}, ${component.actionname}) failed`,
                    );
                }
            })
            .then((data) => {
     ...
     */
    static EditRuleComponent = async (
        rule: Rule,
        component: RuleComponent,
        newValue: string,
    ): Promise<void> => {
        Logger.Info(
            `API.EditRuleComponent(${rule.zone}, ${rule.ruleid}, ${component.componentid})`,
        );

        if (!rule.zone || !rule.ruleid || !component.componentid) {
            throw Error(
                `API.EditRuleComponent(${rule.zone}, ${rule.ruleid}, ${component.componentid}) bad arguments`,
            );
        }
        ...
        return this.put(
            `rules/${rule.zone}/${rule.ruleid}/components/${component.componentid}`,
            putValue,
        ).then((resp) => {
            if (!resp.ok) {
                throw Error(
                    `API.EditRuleComponent(${rule.zone}, ${rule.ruleid}, ${component.componentid}) failed`,
                );
            }
        });
    };
     ...
     * Delete a rule.
     */
    static DeleteRuleComponent = async (
        rule: Rule,
        component: RuleComponent,
    ): Promise<void> => {
        Logger.Info(
            `API.DeleteRuleComponent(${rule.zone}, ${rule.ruleid}, ${component.componentid})`,
        );

        if (!rule.zone || !rule.ruleid || !component.componentid) {
            throw Error(
                `API.DeleteRuleComponent(${rule.zone}, ${rule.ruleid}, ${component.componentid}) bad arguments`,
            );
        }
        ...
        return this.delete(
            ...
            if (!resp.ok) {
                throw Error(
                    `API.DeleteRuleComponent(${rule.zone}, ${rule.ruleid}, ${component.componentid}) failed`,
                );
            }
        });
    };
    ...
    static AddUserToZone = async (
        ...
        if (!zone || !user) {
            throw Error(
                `API.AddUserToZone(${zone}, ${user}, ${permission}) bad arguments`,
            );
        }
        ...
        if (permission == PERMISSION.ASSIGNROLES)
            permissionString = "ASSIGNROLES";
        ...
        try {
            await this.post(`authz/${zone}`, {
                ...
                if (!resp.ok) {
                    throw Error(
                        `API.AddUserToZone(${zone}, ${user}, ${permission}) failed [expected, if old API didn't fail]`,
                    );
                }
                return;
            });
        } catch {
            /* will fail for now (early feb 2025) */
        }
    };
     ...
     */
    static RemoveUserFromZone = async (
        zone: string,
        user: string,
    ): Promise<void> => {
        ...
        if (!zone || !user) {
            throw Error(
                `API.RemoveUserFromZone(${zone}, ${user}) bad arguments`,
            );
        }
    ...
    /**
     * GET /authz/users/all: Get all the users the user has any permission on.
     */
    static GetAllUsers = async (): Promise<string[]> => {
        Logger.Info(`API.GetAllUsers()`);

        return this.get("authz/_users_/all")
            .then((resp) => {
                if (resp.ok) {
                    return resp.json();
                } else {
                    throw Error(`API.GetAllUsers() failed`);
                }
            })
            .then((data) => {
                return data as string[];
