...
const makeHelloWorldRule = async (zone: string): Promise<Rule> => {
    const newRule = await DusseldorfAPI.AddRule(
        zone,
        "http",
        100,
        "GET/POST response with hello world",
    ).catch((err: Error) => {
...
const makeLocalhostRule = async (zone: string): Promise<Rule> => {
    ...
    // IPv6 rule
    const newRule6 = await DusseldorfAPI.AddRule(
        zone,
        "dns",
        100,
        "DNS AAAA response (::1)",
                                ...
                                <div className="stack vstack-gap">
                                    <MessageBar layout="multiline">
                                        <MessageBarBody className="stack vstack-gap">
                                            ...
                                            >
                                                This will create a rule that
                                                responds to all HTTP(s) GET and
                                                POST requests sent to{" "}
                                                <strong>{zone}</strong> with{" "}
                                    ...
                                    <MessageBar layout="multiline">
                                        <MessageBarBody className="stack vstack-gap">
                                            ...
                                            >
                                                This will make 2 rules to make
                                                this DNS zone resolve to{" "}
                                                <strong>localhost</strong>. It
                                                will respond to both both{" "}
                                                <strong>127.0.0.1</strong> and{" "}
                                                <strong>::1</strong> for DNS A
                                                and AAAA requests send to{" "}
                                                <strong>{zone}</strong>.
                                            </Text>
                                        </MessageBarBody>
                                        <MessageBarActions>
